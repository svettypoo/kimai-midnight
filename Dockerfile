...
RUN apt-get update && apt-get install -y --no-install-recommends \
        bash coreutils haveged unzip curl \
        libldap2-dev libicu-dev libpng-dev libzip-dev libxslt1-dev libfreetype6-dev \
        libicu72 libldap-common libpng16-16 libzip4 libxslt1.1 libfreetype6 \
    && rm -rf /var/lib/apt/lists/*

# ── PHP extensions ───────────────────────────────────────────────────
RUN docker-php-ext-configure gd --with-freetype && \
    docker-php-ext-install -j$(nproc) gd intl ldap pdo_mysql zip xsl opcache
...
RUN echo "Listen 8001" > /etc/apache2/ports.conf && \
    ls -la /etc/apache2/mods-enabled/mpm* 2>&1 && \
    a2enmod rewrite && touch /use_apache
